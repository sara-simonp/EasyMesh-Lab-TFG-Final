...
def write_tr181_csv(state: Dict[str, Any]) -> None:
    ...
    for ap_name, ap in sorted(state["aps"].items(), key=lambda kv: kv[1]["device_index"]):
        ...
        for radio in ap["radios"]:
            ...
            # Direct Device.WiFi.Radio mapping requested by the TelcoX BulkData profile.
        ...
        csv_write_param(rows, f"{ap_base}.Enable", "true" if ap["status"] == "Online" else "false", "xsd:boolean")
